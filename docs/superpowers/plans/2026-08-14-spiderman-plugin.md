...
const SHEET_URL = '/plugins/spider-pet/spritesheet.png'
...
export function apply(ctx: ClientContext): void {
  ...
  const syncEnabled = (): void => {
    ...
    if (enabled) {
      ...
      try {
        disposer = mountPet(controller, SPRITE_META, FRAME_TABLE, SHEET_URL)
      } catch (error) {
...
cp /Users/aadmin/tool/蜘蛛侠网页_图片保留_视频可替换/public/home-cover-peter.jpg packages/dsh-skin-spiderman/src/assets/peter.jpg
cp /Users/aadmin/tool/蜘蛛侠网页_图片保留_视频可替换/public/home-cover-suit.jpg packages/dsh-skin-spiderman/src/assets/suit.jpg
```

In `src/client/index.ts`, import `mountReveal` and the two asset URLs (imported as `?url` or copied by the bundle config; tsdown preset inlines small assets via the CSS-modules pipeline — if asset import typing is unavailable, reference `import.meta.url`-relative paths served from `/plugins/ui-skin-spiderman/` and place files under `src/assets` in `files`):
```ts
disposers.push(mountReveal(document.body, {
  peter: '/plugins/ui-skin-spiderman/peter.jpg',
  suit: '/plugins/ui-skin-spiderman/suit.jpg',
}))
```

Note: if the plugin id path differs, derive it from `ctx` plugin id at runtime; keep the literal consistent with `cordis.patch.yml` (`ui-skin-spiderman`).
...
**Interfaces:**
- Produces: `spritesheet.png` 8 columns x 6 rows, 256x256 cells (2048x1536); `pet.json` `{ rows: { idle:0, waiting:1, thinking:2, jumping:3, pet:4, failed:5 }, frames: [6,6,6,5,4,4], cell: { width:256, height:256 } }`.
...
console.log('wrote spritesheet.png and pet.json')
```
...
- [ ] **Step 3: Update client frame table to match pet.json**

In `packages/dsh-spider-pet/src/client/index.ts`, replace the inline `FRAME_TABLE` with a `pet.json` import (add `resolveJsonModule` already in base config) or keep the literal but add a runtime consistency assertion in tests:
```ts
import petJson from '../assets/pet.json'
```
Copy `素材/spidey/pet.json` + `spritesheet.png` into `packages/dsh-spider-pet/src/assets/`, reference `SHEET_URL = '/plugins/spider-pet/spritesheet.png'` and set `FRAME_TABLE = petJson`.
